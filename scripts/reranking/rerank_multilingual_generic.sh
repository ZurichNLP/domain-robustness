...
for domain in $in_domain; do
    ...
    cat $reranked/$model_name/dev.reranked_nbest.$model_name.$domain.$trg | python $scripts/extract_top_translations_from_nbest.py --top 1 > $reranked/$model_name/dev.reranked_best.bpe.tag.$model_name.$domain.$trg

    # remove target language tag

    cat $reranked/$model_name/dev.reranked_best.bpe.tag.$model_name.$domain.$trg | python $scripts/remove_tag_from_translations.py --src-tag "<2$src>" --trg-tag "<2$trg>" > $reranked/$model_name/dev.reranked_best.bpe.$model_name.$domain.$trg
