...
for domain in $in_domain; do
    ...
    cat $translations/$model_name/dev.nbest.$model_name.$domain.$trg | python $scripts/extract_top_translations_from_nbest.py --top 1 > $translations/$model_name/dev.pieces.tag.$model_name.$domain.$trg

    # remove target language tag

    cat $translations/$model_name/dev.pieces.tag.$model_name.$domain.$trg | python $scripts/remove_tag_from_translations.py --src-tag "<2$src>" --trg-tag "<2$trg>" > $translations/$model_name/dev.pieces.$model_name.$domain.$trg
