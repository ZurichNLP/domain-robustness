...
if [[ `hostname` == 'login0' ]]; then
    # S3IT
    base=/net/cephfs/home/mathmu/scratch/domain-robustness-reproduce
...
if [[ `hostname` == 'login0' ]]; then
  # S3IT
  sbatch --qos=vesta --time=36:00:00 --gres gpu:Tesla-V100:1 --cpus-per-task 3 --mem 48g $1 $base
else
  # CSD3
  sbatch -A T2-CS037-GPU --gres=gpu:1 --nodes=1 --time=168:00:00 --cpus-per-task 3 -p pascal $1 $base
fi;
